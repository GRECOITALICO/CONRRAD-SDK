import shutil
"""
CONRRAD SDK — Containerized Execution & Resource Management
══════════════════════════════════════════════════════════════
...
logger = logging.getLogger("conrrad.sandbox")

# Docker image digest for supply chain verification
AGENT_BASE_IMAGE_TAG = "conrrad/agent-base:latest"  # Para referencia humana
# ↓ Usar este en producción (inmutable, no puede ser reemplazado silenciosamente)
AGENT_BASE_IMAGE = "conrrad/agent-base@sha256:34a57c2f12b3b22305db83db1e2aa58fb12907dd428d6f22f3ebc6e4e260d926"
...
class Sandbox:
    ...
    def __init__(self, agent_id: str, limits: ResourceLimits, permissions: AgentPermissions):
        ...
        self.container_name = f"conrrad_agent_{self.agent_id}"
    ...
    def _build_docker_args(self) -> List[str]:
        ...
        import os
        auth_key = os.environ.get("CONRRAD_SANDBOX_AUTH_KEY")
        if auth_key:
             args.extend(["-e", f"CONRRAD_SANDBOX_AUTH_KEY={auth_key}"])
