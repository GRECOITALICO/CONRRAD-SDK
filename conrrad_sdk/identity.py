"""
CONRRAD SDK — Identity & Passport System
═══════════════════════════════════════════════
Every agent created with the SDK receives a unique, cryptographic
identity (Passport) that makes it a citizen of the Conrrad Ecosystem.
...
import logging
logger = logging.getLogger("conrrad.identity")
...
def _get_machine_secret() -> str:
    ...
    secret_path = Path.home() / ".conrrad" / ".machine_secret"
    ...
    warnings.warn(
        "Machine secret stored on filesystem at ~/.conrrad/.machine_secret. "
...
@dataclass
class AgentPassport:
    """Immutable identity document for a CONRRAD Agent."""
    agent_id: str
    name: str
    version: str
    created_at: float
    ...
    # Conrrad ecosystem registration
...
class HistoricalEncryptionDetected(Exception):
    pass
...
def _decrypt_private_key(encrypted_data: bytes, storage_dir: Path) -> str:
    ...
    if encrypted_data.startswith(b"gAAAAA"):
        raise HistoricalEncryptionDetected()
...
def load_private_key(storage_dir: Path) -> Optional[str]:
    ...
    if key_path.exists():
        ...
        try:
        ...
        except HistoricalEncryptionDetected:
            ...
            return private_hex
    # Historical fallback: check for unencrypted key and migrate
...
class IdentityRegistry:
    ...
    def __init__(self, redis_client, prefix: str = "conrrad:identity"):
        self.r = redis_client
        self.prefix = prefix
