...
import sys
import json
from conrrad_sdk.security.sandbox.seccomp_profile import install_seccomp
...
def main():
    ...
    from conrrad_sdk.security.intent_firewall import OrchestratorStub, PlanIR, CapabilityToken
    
    plan = PlanIR(**payload["plan"])
    token = CapabilityToken(**payload["token"])
    
    auth_key_b64 = os.environ.get("CONRRAD_SANDBOX_AUTH_KEY")
    if not auth_key_b64:
        print(json.dumps({"error": "CONRRAD_SANDBOX_AUTH_KEY environment variable is required"}))
        sys.exit(1)
    ...
    try:
        ...
        from conrrad_sdk.security.intent_firewall import TokenAuthority
        authority = TokenAuthority(private_key)
    except Exception as e:
        print(json.dumps({"error": f"Invalid CONRRAD_SANDBOX_AUTH_KEY: {e}"}))
        sys.exit(1)
