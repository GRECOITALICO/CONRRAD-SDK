"""
Conrrad Pay — Persistent Redis Store (Nivel 1 Production)
═════════════════════════════════════════════════════════
...
logger = logging.getLogger("conrrad.redis_store")

class RedisTenantStore:
    def __init__(self, redis_url: str = "redis://localhost:6379/0"):
        self.client = redis.from_url(redis_url, decode_responses=True)
        # Prefix keys to avoid collisions
        self.prefix = "conrrad:tenant"
    ...
    def check_or_set_processing(self, idempotency_key: str) -> str:
        ...
        key = f"conrrad:idemp:{idempotency_key}"
    ...
    def settle_funds(self, tenant_id: str, reserved_usd: float, actual_usd: float, transaction_id: str) -> bool:
        ...
        """
        settled_key = f"conrrad:settled:{transaction_id}"
