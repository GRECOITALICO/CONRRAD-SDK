...
logger = logging.getLogger("conrrad.subprocess_runtime")


def _is_production_env() -> bool:
    return os.getenv("CONRRAD_ENV", "").strip().lower() == "production"
...
class SubprocessRuntime(BaseRuntime):
    ...
    def __init__(self, allow_insecure_exec: bool = False, *args, **kwargs):
        if _is_production_env():
            raise SecurityError(
                "SubprocessRuntime is forbidden when CONRRAD_ENV=production "
        ...
        logger.warning(
            ...
            extra={"allow_insecure_exec": True, "CONRRAD_ENV": os.getenv("CONRRAD_ENV", "unset")},
        )
    ...
    def _drop_privileges(self):
        ...
        if hasattr(os, 'getuid') and os.getuid() == 0:
            try:
                ...
                # En un entorno real, crearíamos un usuario 'conrrad-jail'
                os.setgid(65534)
                os.setuid(65534)
            except OSError as e:
                import logging
    ...
    def execute(self, request: ExecutionRequest) -> ExecutionResult:
        if _is_production_env():
            raise SecurityError("SubprocessRuntime.execute blocked: CONRRAD_ENV=production.")
        if os.environ.get("CONRRAD_ALLOW_UNSAFE_SUBPROCESS_RUNTIME") != "1":
            raise RuntimeError(
                "SubprocessRuntime.execute blocked: set CONRRAD_ALLOW_UNSAFE_SUBPROCESS_RUNTIME=1 "
