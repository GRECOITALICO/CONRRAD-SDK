"""
Conrrad Pay — Execution Proxy Core (Nivel 1)
════════════════════════════════════════════
...
logger = logging.getLogger("conrrad.proxy")
...
class ConrradExecutionProxy:
    def __init__(self, store: Any, audit_log_path: Optional[str] = "/var/log/conrrad/receipts.jsonl"):
        self.store = store
    ...
    def execute_stream(
        ...
        if idemp_state == "COMPLETED":
            ...
            yield f"[CONRRAD_PAY: CACHED] {json.dumps(asdict(receipt))}"
            return
        elif idemp_state == "PROCESSING":
            yield "[CONRRAD_PAY: 429 CONFLICT - TRANSACTION IN PROGRESS]"
            return
        ...
        try:
            ...
            if not self.store.reserve_funds(tenant_id, effective_budget_usd):
                yield "[CONRRAD_PAY: 402 PAYMENT REQUIRED - INSUFFICIENT BUDGET]"
                return
    ...
    def _write_receipt(self, receipt: 'ExecutionReceipt'):
        ...
        ConrradExecutionProxy._receipts_emitted += 1
        ...
        try:
            ...
            with open(self.audit_log_path, "a") as f:
                ...
                f.flush()
                os.fsync(f.fileno())
            ConrradExecutionProxy._receipts_written += 1
    ...
    def _record_settlement_latency(self, pending_ts: float):
        ...
        ConrradExecutionProxy._settlement_latencies_ms.append(latency_ms)
        if latency_ms > 500:
            logger.warning(
