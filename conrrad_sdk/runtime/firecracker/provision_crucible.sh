...
echo "=========================================================="
echo "🔥 Provisioning Conrrad OS Crucible Node (Bare Metal/KVM)"
echo "=========================================================="
...
mkdir -p /etc/systemd/system/conrrad.service.d
cat <<EOF > /etc/systemd/system/conrrad.service.d/override.conf
[Service]
LimitNOFILE=1000000
EOF
...
cat <<EOF > /etc/sysctl.d/99-conrrad.conf
...
echo "[5/6] Creating Conrrad Data Directories..."
mkdir -p /var/lib/conrrad
mkdir -p /var/log/conrrad
mkdir -p /tmp/fcsnapshots
chown -R $SUDO_USER:$SUDO_USER /var/lib/conrrad /var/log/conrrad /tmp/fcsnapshots
echo "✅ Directories created."

echo "[6/6] Installing Conrrad OS SDK & Server..."
...
echo "=========================================================="
echo "Next Steps:"
echo "1. Compile kernel & rootfs: cd conrrad_sdk/runtime/firecracker && sudo ./build_rootfs.sh"
echo "2. Move vmlinux & rootfs to /var/lib/conrrad/"
echo "3. Start the Control Plane: .venv/bin/python3 -m conrrad_sdk.runtime.firecracker.server"
