"""
CONRRAD Firecracker Control Plane — HTTP server.
...
# This prevents ImportError when conrrad_sdk is imported in environments
...
async def _lifespan(application):
    ...
    # ── Startup ──────────────────────────────────────────────────────────────
    kernel_path = os.getenv("FC_KERNEL", "/var/lib/conrrad/vmlinux")
    rootfs_path = os.getenv("FC_ROOTFS", "/var/lib/conrrad/rootfs.ext4")
...
app = FastAPI(title="CONRRAD Firecracker Control Plane", lifespan=_lifespan)
...
def main():
    if not _WEB_AVAILABLE:
        raise ImportError(
            ...
            "Install them with: pip install 'conrrad[gui]'"
        )
    ...
    uvicorn.run("conrrad_sdk.runtime.firecracker.server:app", host=host, port=port, log_level="warning")
