"""
CONRRAD SDK — Command Center Dashboard
══════════════════════════════════════════
...
logger = logging.getLogger("conrrad.dashboard")
...
class CommandCenter:
    ...
    def __init__(self, agent, port: int = 8500,
        ...
        self.app = FastAPI(title=f"Conrrad Command Center — {agent.name}")
        self.app.add_middleware(
            CORSMiddleware,
            allow_origins=[
                "https://app.conrrad.ai",
...
{ROUTER_DASHBOARD_JS}
    ...
    def start(self):
        if not HAS_DEPS:
            logger.error("Install: pip install conrrad-sdk[gui]")
            return
        print(f"\n  🎯 CONRRAD COMMAND CENTER")
        print(f"  ════════════════════════════════════")
