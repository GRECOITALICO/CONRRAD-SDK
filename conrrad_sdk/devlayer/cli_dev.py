"""
CONRRAD — DevLayer CLI
══════════════════════════
Developer-facing CLI commands for the Conrrad distributed coding fabric.

Commands:
  conrrad dev index   - Index the current codebase for context routing
  conrrad dev ask     - Submit a coding task to the Conrrad network
  conrrad dev review  - Review pending execution receipts (accept/reject)
  conrrad dev status  - Show current task pipeline status
  conrrad dev history - Show task execution history

Usage:
  $ conrrad dev index
  $ conrrad dev ask "refactor the auth module to use JWT tokens"
  $ conrrad dev review
"""
import os
import sys
...
logger = logging.getLogger("conrrad.devlayer.cli")


def _get_project_root() -> str:
    """Find the project root (directory with conrrad.yaml or .git)."""
    cwd = Path.cwd()
    for marker in ["conrrad.yaml", ".git", "pyproject.toml", "package.json"]:
...
def cmd_dev(args):
    ...
    if not sub:
        print("\n  ⬡ Conrrad DevLayer — Available Commands:")
        print("  ─────────────────────────────────────────")
        print("  conrrad dev index    Index the codebase")
        print("  conrrad dev ask      Submit a coding task")
        print("  conrrad dev review   Review pending results")
        print("  conrrad dev status   Pipeline status")
        print("  conrrad dev history  Execution history")
        print()
        return
...
def _cmd_index(args):
    ...
    from conrrad_sdk.devlayer.context_router import ContextRouter
    from conrrad_sdk.devlayer.preview import PreviewEngine
    ...
    print("  ✅ Index cached at .conrrad/index.json")
    print(f"  Ready to route context to {summary['total_files']} files\n")


def _cmd_ask(args):
    """Submit a coding task to the Conrrad network."""
    from conrrad_sdk.devlayer.context_router import ContextRouter
    from conrrad_sdk.devlayer.task_client import TaskClient
    from conrrad_sdk.devlayer.preview import PreviewEngine

    description = getattr(args, "description", None)
    if not description:
        print("  ❌ Usage: conrrad dev ask \"<task description>\"")
        return
    ...
    if task.receipts:
        ...
        try:
            ...
            if choice == "a":
                ...
                print("\n  ⏭️  Skipped. Use 'conrrad dev review' to review later.\n")
        except (EOFError, KeyboardInterrupt):
            print("\n  ⏭️  Skipped.\n")
    else:
        print("  ⚠️  No receipts received. Task may still be executing.")
        print(f"  Track with: conrrad dev status --task {task.task_id}\n")
...
def _cmd_status(args):
    ...
    if total == 0:
        print("\n  📭 No active tasks. Use 'conrrad dev ask' to submit one.\n")
        return
...
def _cmd_history(args):
    ...
    project_root = _get_project_root()
    history_path = Path(project_root) / ".conrrad" / "task_history.jsonl"
