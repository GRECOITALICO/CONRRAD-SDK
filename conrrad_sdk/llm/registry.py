"""
CONRRAD SDK — LLM Provider Registry
════════════════════════════════════════
Unified entry point for ALL LLM calls in the SDK.
Ported from CONRRAD monorepo core/sea/llm_registry.py and adapted
...
Usage:
    from conrrad_sdk.llm.registry import LLMProviderRegistry
...
logger = logging.getLogger("conrrad.llm.registry")
...
class LLMProviderRegistry:
    ...
    Ported from CONRRAD monorepo core/sea/llm_registry.py.
    """
    ...
    def _call_openai_compat(self, config, api_key, messages, system_prompt, max_tokens, temperature):
        ...
        if config.get("openrouter"):
            h["HTTP-Referer"] = "https://conrrad.os"
            h["X-Title"] = "CONRRAD SDK"
    ...
    def _post(self, url: str, payload: dict, headers: dict) -> Optional[dict]:
        ...
        req = urllib.request.Request(
            url, data=data,
            headers={**headers, "User-Agent": "Conrrad-SDK/3.0"},
            method="POST"
        )
        try:
