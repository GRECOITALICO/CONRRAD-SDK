...
from conrrad_sdk.security.ssrf import create_safe_client, RequestError, TimeoutException
...
def write_secure_env_file(env_vars: dict[str, str], path: str = ".env") -> Path:
    ...
    lines = [
        "# CONRRAD SDK — Archivo de configuración seguro\n",
...
app = FastAPI(title="CONRRAD - Setup Wizard")
        ...
        with create_safe_client(agent_id="launcher_wizard", timeout=8.0) as client:
            resp = client.get(
                ...
                headers={
                    ...
                    "User-Agent": "conrrad-sdk-installer",
                }
            )
...
def serve_wizard():
    ...
    <head>
        <meta charset="UTF-8">
        <title>CONRRAD - Unified Installer</title>
        <style>
    ...
    <body>
        <div class="card">
            <h1>CONRRAD Installation</h1>
            <p class="subtitle">Unified installer for Conrrad Agent Swarm & Conrrad Pay</p>
            ...
            <div class="github-box">
                ...
                <p style="font-size: 0.75rem; color: #cbd5e1; margin-bottom: 10px;">CONRRAD is free. To proceed, please <a href="https://github.com/GRECOITALICO/CONRRAD-SDK" target="_blank" style="color: #38bdf8;">Star our GitHub repo</a>.</p>
            ...
            <div class="checkbox-group">
                ...
                <label style="margin:0;">Enable Conrrad Pay Protocol (Dual Wallet)</label>
            </div>
            ...
            async function submitSetup() {
                ...
                const resp = await fetch('/api/setup', {
                    ...
                    body: JSON.stringify({
                        ...
                        openai_key: "",
                        enable_conrrad_pay: document.getElementById('enablePay').checked,
                        stripe_key: "",
                        strict_sandbox: true,
...
def handle_setup(data: SetupData, request: Request):
    ...
    if data.enable_conrrad_pay:
        ...
        _write_secret_file(
            ".conrrad/tx_private.key",
            private_key_hex
        )
        print("\n" + "=" * 60)
        print("  ⚠️  CLAVE PRIVADA GENERADA")
        print(f"  Ubicación: .conrrad/tx_private.key")
    ...
    env_content = f"""# CONRRAD SDK — Configuración generada automáticamente
# NUNCA subir a control de versiones

ANTHROPIC_API_KEY={data.anthropic_key}
OPENAI_API_KEY={data.openai_key}
CONRRAD_CLUSTER_NAME={data.swarm_name}_cluster
REDIS_URL=redis://localhost:6379
STRICT_SANDBOX={str(data.strict_sandbox).lower()}
CONRRAD_PAY_ENABLED={str(data.enable_conrrad_pay).lower()}
CONRRAD_PUBLIC_ADDRESS={public_address}
CONRRAD_VOLATILE_ADDRESS={volatile_address}
# CONRRAD_TX_PRIVATE_KEY — NO está aquí, ver: .conrrad/tx_private.key
"""
...
from conrrad_sdk import Agent, AgentPermissions
from conrrad_sdk.llm import LLMRouter, OllamaProvider, AnthropicProvider
from conrrad_sdk.cluster import ClusterDiscovery

load_dotenv()

def main():
    print("🚀 Booting CONRRAD Infrastructure...")
    print(f"🤖 Swarm Name: {swarm_name}")
    
    if {enable_pay}:
        print("💳 Conrrad Pay: L1+L2 Dual Wallet Enabled")
...
"""
    ...
    content = MAIN_TEMPLATE.format(
        ...
        enable_pay=repr(bool(data.enable_conrrad_pay)),
...
def run_launcher():
    ...
    if not os.path.exists(".env") or not os.path.exists("main.py"):
        print("=" * 60)
        print("  CONRRAD OS — WEB SETUP WIZARD")
