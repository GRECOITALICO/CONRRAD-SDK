...
class ConrradConfig(BaseModel):
    """Configuration for the CONRRAD SDK."""

    model_config = ConfigDict(env_prefix="CONRRAD_")

    api_key: str = Field(default_factory=lambda: os.getenv("CONRRAD_API_KEY", ""))
    gateway_url: str = Field(default_factory=lambda: os.getenv("CONRRAD_GATEWAY_URL", "https://api.conrrad.online"))
    ...
    def validate_gateway_url(cls, v: str) -> str:
        ...
        if os.getenv("CONRRAD_ENV", "development") == "production":
    ...
    redis_url: Optional[str] = Field(default_factory=lambda: os.getenv("CONRRAD_REDIS_URL", None))
    environment: str = Field(default_factory=lambda: os.getenv("CONRRAD_ENV", "development"))

    # Multi-tenant binding for signed channels (A2A, audit); default is single-tenant alpha.
    tenant_id: str = Field(default_factory=lambda: os.getenv("CONRRAD_TENANT_ID", "default"))

    # Wallet / Escrow configuration
    wallet_address: Optional[str] = Field(default_factory=lambda: os.getenv("CONRRAD_WALLET_ADDRESS", None))
    # SECURITY: This field is EXCLUDED from serialization
    wallet_private_key: Optional[str] = Field(
        default_factory=lambda: os.getenv("CONRRAD_WALLET_KEY", None),
...
# ── Lazy singleton ────────────────────────────────────────────────────────────
# Instantiating ConrradConfig() at module import time means any env vars set
...
def get_default_config() -> "ConrradConfig":
    """Return the process-wide default ConrradConfig (lazy, cached).

    Call ``get_default_config.cache_clear()`` in tests to reset between runs.
    """
    return ConrradConfig()


class _LazyConfig:
    """Proxy that behaves like ConrradConfig but defers construction."""
...
# Keep the name so existing `from conrrad_sdk.config import default_config` works.
default_config: "ConrradConfig" = _LazyConfig()  # type: ignore[assignment]
