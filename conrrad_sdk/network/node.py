...
from conrrad_sdk.network.peer_manager import PeerManager
from conrrad_sdk.network.gossip import GossipLayer
from conrrad_sdk.network.validator import ProtocolValidator
from conrrad_sdk.network.event_log import EventLog, Event
from conrrad_sdk.network.state_sync import StateSync, Metrics

logger = logging.getLogger("p2p.node")

class P2PNode:
    """
    The Product P2P Node combining PeerManager, Gossip, Validator, and StateSync.
    """
    def __init__(self, pubkey: str, redis_url: str):
        ...
        self.quorum_size = 2  # Note: assume 3 total nodes -> quorum = 2
    ...
    def _process_valid_message(self, msg: Dict[str, Any]):
        if msg["type"] == "STATE_UPDATE":
            ...
            if event_data:
                ...
                # but for product, we only trust local confirmations.
