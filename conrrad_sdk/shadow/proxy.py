"""
CONRRAD SDK — Shadow Proxy (OpenAI-Compatible Interceptor)
═════════════════════════════════════════════════════════════
Transparent observation layer for existing AI workloads.

How it works:
  1. Client installs conrrad-sdk
  2. Changes ONE line: `from conrrad_sdk.shadow import openai`
  ...
  5. We compute: what Conrrad WOULD have done (counterfactual)
  6. Dashboard shows: "You spent $X. Conrrad would have spent $Y."
...
logger = logging.getLogger("conrrad.shadow")
...
class ShadowConfig:
    ...
    org_id: str = ""
    log_dir: str = ""
    telemetry_endpoint: str = "https://api.conrrad.ai/telemetry"
    ...
    def __post_init__(self):
        ...
        if not self.log_dir:
            self.log_dir = str(Path.home() / ".conrrad" / "shadow")
...
# Counterfactual: what Conrrad would route to
CONRRAD_ROUTING_TABLE = {
...
class ShadowEvent:
    ...
    # What Conrrad WOULD have done
    conrrad_model: str
    conrrad_tier: str
    conrrad_cost_usd: float
    conrrad_confidence: float
...
class ShadowProxy:
    ...
    Usage (Nivel 1 — SDK Wrapper):
        from conrrad_sdk.shadow import ShadowProxy
    ...
    Usage (Nivel 2 — Monkey-patch):
        from conrrad_sdk.shadow.proxy import patch_openai
    ...
    def observe(
        ...
        # Compute counterfactual (what Conrrad would have done)
        conrrad_model, conrrad_tier = CONRRAD_ROUTING_TABLE.get(
            complexity, ("deepseek-chat", "cheap_api")
        )
        conrrad_cost = self._compute_cost(conrrad_model, t_in, t_out)

        # Compute savings
        savings_usd = max(0, original_cost - conrrad_cost)
        ...
        event = ShadowEvent(
            ...
            conrrad_model=conrrad_model,
            conrrad_tier=conrrad_tier,
            conrrad_cost_usd=round(conrrad_cost, 6),
            conrrad_confidence=self._estimate_confidence(complexity),
        ...
        self._total_conrrad_cost += conrrad_cost
    ...
    def get_dashboard_data(self) -> dict:
        ...
        optimized = self._total_conrrad_cost
        ...
        conf_sum = sum(e.conrrad_confidence for e in events)
        avg_confidence = conf_sum / total_events

        # Savings at risk (from low-confidence routes)
        at_risk = sum(
            e.savings_usd for e in events if e.conrrad_confidence < 0.85
        )

        # Routing distribution
        local_count = sum(1 for e in events if e.conrrad_tier.startswith("local"))
        cheap_count = sum(1 for e in events if e.conrrad_tier == "cheap_api")
        premium_count = sum(1 for e in events if e.conrrad_tier == "premium_api")

        local_cost = sum(e.conrrad_cost_usd for e in events if e.conrrad_tier.startswith("local"))
        cheap_cost = sum(e.conrrad_cost_usd for e in events if e.conrrad_tier == "cheap_api")
        premium_cost = sum(e.conrrad_cost_usd for e in events if e.conrrad_tier == "premium_api")

        local_conf = (
            sum(e.conrrad_confidence for e in events if e.conrrad_tier.startswith("local"))
            / max(1, local_count)
        )
        cheap_conf = (
            sum(e.conrrad_confidence for e in events if e.conrrad_tier == "cheap_api")
            / max(1, cheap_count)
        )
        premium_conf = (
            sum(e.conrrad_confidence for e in events if e.conrrad_tier == "premium_api")
            / max(1, premium_count)
        )
            ...
            "recent_events": [
                {
                    ...
                    "conrrad_route": e.conrrad_model,
                    "actual_cost": e.original_cost_usd,
                    "counterfactual_cost": e.conrrad_cost_usd,
                    "savings": e.savings_usd,
                    "confidence": e.conrrad_confidence,
                }
    ...
    def stats(self) -> dict:
        return {
            ...
            "total_conrrad_cost": round(self._total_conrrad_cost, 4),
