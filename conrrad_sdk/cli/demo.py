...
def run_demo():
    print("\n🚀 CONRRAD Demo — Cost & Intelligence Comparison\n")

    try:
        from conrrad_sdk.router import IntelligentRouter
        from conrrad_sdk.router import TelemetryCollector, TelemetryConfig
        from conrrad_sdk.internal.paths import default_demo_buffer_dir
    ...
    telemetry = TelemetryCollector(
        config=TelemetryConfig(
            enabled=True,
            consent_given=True,
            buffer_dir=str(default_demo_buffer_dir()),
        )
    )
    ...
    total_conrrad_cost = 0.0
    total_baseline_latency = 0.0
    total_conrrad_latency = 0.0
    success_count = 0
    results_summary = []
    ...
    for difficulty, prompt in tasks:
        ...
        conrrad_cost = estimate_cost(route)

        total_baseline_cost += baseline_cost
        total_conrrad_cost += conrrad_cost
        total_baseline_latency += baseline_latency
        total_conrrad_latency += conrrad_latency

        savings = baseline_cost - conrrad_cost

        print(f"Baseline → cost: ${baseline_cost:.3f}, latency: {baseline_latency:.2f}s")
        print(f"Conrrad  → route: {route}, cost: ${conrrad_cost:.3f}, latency: {conrrad_latency:.2f}s")
        ...
        results_summary.append({
            ...
            "conrrad_cost": conrrad_cost,
            "savings": savings,
        })

    total_savings = total_baseline_cost - total_conrrad_cost
    savings_pct = (total_savings / total_baseline_cost) * 100 if total_baseline_cost else 0
    latency_delta = ((total_conrrad_latency - total_baseline_latency) / total_baseline_latency * 100) if total_baseline_latency else 0
    ...
    print(f"Conrrad cost:  ${total_conrrad_cost:.3f}")
...
def main() -> int:
    ...
    print("\n─────────────────────────\n")
    print("Conrrad is optimizing your AI stack in real time.\n")
