...
from conrrad_sdk.internal.paths import (
    default_executions_file,
    default_tasks_file,
...
def _core_api_base() -> str:
    return os.environ.get("CONRRAD_CORE_URL", "http://127.0.0.1:8000").rstrip("/")
...
def _executions_path() -> Path:
    return default_executions_file()
...
def version_upgrade():
    subprocess.Popen([
        sys.executable, "-m", "pip", "install", "--upgrade", "conrrad-sdk"
    ])
...
def dashboard_run_v1():
    ...
    DevLayer v1: save/list/reuse tasks (JSON file under ~/.conrrad/data).
    Execution history v1: append-only local log under ~/.conrrad/data/executions.json.
    """
...
<body>
  <h1>Conrrad Dashboard — Run (v1)</h1>
  <p class="hint">Calls Core <code>POST /api/v1/sandbox/execute-v2</code> via same-origin proxy. Core default: <code>{core_hint}</code> (set <code>CONRRAD_CORE_URL</code> to override).</p>

  <label for="apiKey">API key</label>
  <input id="apiKey" type="text" autocomplete="off" placeholder="sk_test_conrrad_..." />
  ...
  <p class="hint">Stored locally under <code>~/.conrrad/data/tasks.json</code> (override with <code>CONRRAD_TASKS_PATH</code>).</p>
  <div id="tasks"></div>

  <h2 style="margin-top:2rem;font-size:1.1rem;">Execution history</h2>
  <p class="hint">Recent runs (newest first in this list). File: <code>~/.conrrad/data/executions.json</code> (<code>CONRRAD_EXECUTIONS_PATH</code>).</p>
  ...
  <script>
    ...
    function formatOutput(data) {{
      ...
      const econ =
        ...
        "[Estimate]   " + est.toFixed(6) + " SEAL\\n" +
        "[Actual]     " + act.toFixed(6) + " SEAL\\n" +
        "[Refund]     " + refund.toFixed(6) + " SEAL\\n" +
        "[Remaining]  " + remaining.toFixed(6) + " SEAL\\n" +
        "Saved vs estimate: " + saved.toFixed(6) + " SEAL\\n";
      ...
      try {{
        ...
        rows.forEach((r) => {{
          ...
          if (ok) {{
            ...
            line.textContent = prefix + "✔ " + name + tt + act + " SEAL";
          }} else {{
    ...
    async function runSaved(taskId) {{
      ...
      try {{
        ...
        if (err.name === "AbortError") {{
          output.textContent = "❌ Execution timed out after " + (RUN_TIMEOUT_MS/1000) + "s. Check Core at CONRRAD_CORE_URL.";
        }} else {{
      ...
      try {{
        ...
        if (!res.ok) {{
          ...
          if (res.status === 502 && data.core_url) {{
            throw new Error(msg + " — check CONRRAD_CORE_URL (" + data.core_url + ")");
          }}
          throw new Error(msg);
        }}
        output.textContent = formatOutput(data);
...
async def proxy_api(request: Request, path: str):
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            return Response(content=json.dumps({"error": str(e), "message": "CONRRAD API Server not running on 8502"}), status_code=502)
