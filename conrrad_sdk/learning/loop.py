"""
CONRRAD SDK — Closed Learning Loop (Portable)
══════════════════════════════════════════════════
SDK-portable version of the Closed Learning Loop engine.
Designed to work standalone (filesystem) or with Redis.

Unlike the core/learning version (which is CONRRAD-specific),
this module can be used by any project that installs conrrad SDK.

Usage:
    from conrrad_sdk.learning.loop import LearningLoop, TaskTrace
...
logger = logging.getLogger("conrrad.sdk.learning")
...
class LearningLoop:
    ...
    def get_stats(self) -> Dict:
        ...
        if self.redis:
            raw = self.redis.hgetall("conrrad:learning:stats") or {}
...
> Auto-generated by CONRRAD SDK Closed Learning Loop.
...
"""
    ...
    def _index_skill(self, skill_id: str, trace: TaskTrace, wf: Dict):
        if not self.redis:
            return
        self.redis.hset("conrrad:learning:skills_index", skill_id, json.dumps({
    ...
    def _persist_trace(self, trace: TaskTrace):
        if not self.redis:
            return
        self.redis.lpush("conrrad:learning:task_traces", json.dumps(trace.to_dict()))
        self.redis.ltrim("conrrad:learning:task_traces", 0, 199)
        self._update_stats("traces_processed")

    def _update_stats(self, key: str, delta: int = 1):
        if self.redis:
            self.redis.hincrby("conrrad:learning:stats", key, delta)
