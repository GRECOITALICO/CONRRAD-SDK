"""
CONRRAD SDK — Configuration Loader
══════════════════════════════════════
Loads `conrrad.yaml` and instantiates the entire Cognitive Layer:
...
Usage:
    from conrrad_sdk.cognitive.config_loader import load_config
    env = load_config("conrrad.yaml")
...
logger = logging.getLogger("conrrad.cognitive.config")


@dataclass
class ConrradEnvironment:
    """Fully initialized CONRRAD runtime environment."""
    project_name: str
    workspace: str
...
def load_config(config_path: str = "conrrad.yaml") -> ConrradEnvironment:
    """
    Load a conrrad.yaml and return a fully initialized environment.
    ...
    return ConrradEnvironment(
        project_name=project.get("name", "conrrad-project"),
