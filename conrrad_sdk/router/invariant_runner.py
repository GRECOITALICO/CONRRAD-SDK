...
from conrrad_sdk.router.simulation_engine import IncidentReplayer, StateValidator

logger = logging.getLogger("conrrad.runtime.invariants")
...
class RequestDiscovery:
    ...
    def sample(self, limit=1000) -> List[str]:
        ids = set()
        entries = self.r.xrevrange("conrrad:wal", count=limit)

        for _, data in entries:
            rid = data.get("request_id")
            if not rid:
                continue
            if rid.startswith("conrrad:exec:"):
                rid = rid.split("conrrad:exec:")[1]
            ids.add(rid)
...
def run_ci_pipeline(redis_client=None, limit=500):
    ...
    from conrrad_sdk.router.simulation_engine import WALEventAdapter
