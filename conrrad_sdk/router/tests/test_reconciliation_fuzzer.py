...
from conrrad_sdk.router.reconciler import (
    ReconciliationEngine,
    ReconciliationContext,
    ReconciliationResult,
    reconcile_from_timelines
)
from conrrad_sdk.router.reconciliation_executor import ReconciliationExecutor
...
class ReconciliationFuzzer:
    ...
    def run_all(self):
        print("=" * 70)
        print("  RECONCILIATION FUZZER — CONRRAD")
        print("=" * 70)
    ...
    def test_executor_freeze(self):
        ...
        # Pre-seed state
        for r in [ra, rb]:
            r.hset(f"conrrad:exec:{rid}", mapping={"epoch": "3", "state": "IN_PROGRESS"})
        ...
        self.assert_eq("Executor FREEZE → A state", ra.hgetall(f"conrrad:exec:{rid}")["state"], "FROZEN")
        self.assert_eq("Executor FREEZE → B state", rb.hgetall(f"conrrad:exec:{rid}")["state"], "FROZEN")
    ...
    def test_executor_force_sync(self):
        ...
        ra.hset(f"conrrad:exec:{rid}", mapping={"epoch": "5", "state": "COMPLETED", "result_ptr": "res-123"})
        rb.hset(f"conrrad:exec:{rid}", mapping={"epoch": "3", "state": "IN_PROGRESS"})
        ...
        a_state = ra.hgetall(f"conrrad:exec:{rid}")
        b_state = rb.hgetall(f"conrrad:exec:{rid}")
    ...
    def test_executor_compensate(self):
        ...
        ra.hset(f"conrrad:exec:{rid}", mapping={"epoch": "4", "state": "COMPLETED"})
        rb.hset(f"conrrad:exec:{rid}", mapping={"epoch": "4", "state": "COMPLETED"})
        ...
        self.assert_eq("COMPENSATE → WAL emitted A", len(ra.streams.get("conrrad:wal", [])), 1)
        self.assert_eq("COMPENSATE → WAL emitted B", len(rb.streams.get("conrrad:wal", [])), 1)
    ...
    def test_freeze_idempotency(self):
        ...
        for r in [ra, rb]:
            r.hset(f"conrrad:exec:{rid}", mapping={"epoch": "2", "state": "IN_PROGRESS"})
        ...
        self.assert_eq("Double FREEZE → state still FROZEN", ra.hgetall(f"conrrad:exec:{rid}")["state"], "FROZEN")
    ...
    def test_force_sync_wal_emission(self):
        ...
        ra.hset(f"conrrad:exec:{rid}", mapping={"epoch": "3", "state": "COMPLETED"})
        rb.hset(f"conrrad:exec:{rid}", mapping={"epoch": "1", "state": "IN_PROGRESS"})
        
        executor = ReconciliationExecutor(ra, rb)
        executor.execute(rid, {"action": "FORCE_SYNC_B", "winner": "A", "reason": "test"})
        
        wal_a = ra.streams.get("conrrad:wal", [])
        wal_b = rb.streams.get("conrrad:wal", [])
