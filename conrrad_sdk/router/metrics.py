"""
CONRRAD SDK — Router Metrics & Observability
══════════════════════════════════════════════════
...
logger = logging.getLogger("conrrad.router.metrics")
...
class RouterMetricsCollector:
    ...
    def export_prometheus(self) -> str:
        ...
        def gauge(name, value, help_text="", labels=None):
            if help_text:
                lines.append(f"# HELP conrrad_{name} {help_text}")
                lines.append(f"# TYPE conrrad_{name} gauge")
            label_str = ""
            if labels:
                label_str = "{" + ",".join(f'{k}="{v}"' for k, v in labels.items()) + "}"
            lines.append(f"conrrad_{name}{label_str} {value}")
