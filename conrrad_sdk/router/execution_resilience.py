...
from conrrad_sdk.budget_killer import ExecutionContext, BudgetExceededError
from conrrad_sdk.resilience import CircuitBreakerRegistry
from conrrad_sdk.internal.paths import default_payload_spill_dir

logger = logging.getLogger("conrrad.runtime.protocol")
...
class DistributedELPStore:
    def __init__(self, redis_client=None, lease_ttl=5.0):
        ...
        self.wal_key = "conrrad:wal"
        self.outbox_key = "conrrad:outbox"
    ...
    def _key(self, request_id):
        return f"conrrad:exec:{request_id}"

    def _save_payload_external(self, request_id: str, payload: Any) -> str:
        spill_dir = default_payload_spill_dir()
        spill_dir.mkdir(parents=True, exist_ok=True)
        ptr = spill_dir / f"{request_id}.json"
...
class WALReplayEngine:
    def __init__(self, redis_client=None):
        if not redis_client:
            redis_client = redis.Redis(host="localhost", port=6379, decode_responses=True)
        self.r = redis_client
        self.stream = "conrrad:wal"
        self.meta_key = "conrrad:snapshot:meta"
        self.state_prefix = "conrrad:snapshot:state:"
...
class RuntimeBootstrap:
    ...
    def bootstrap(self):
        ...
        last_id = self.r.get("conrrad:snapshot:meta") or "0-0"

        entries = self.r.xrange("conrrad:wal", min=f"({last_id}")
        ...
        while True:
            ...
            for stream_name, messages in events:
                for entry_id, fields in messages:
                    ...
                    idem_key = f"conrrad:outbox:processed:{req}"
