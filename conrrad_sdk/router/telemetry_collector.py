"""
CONRRAD SDK — Distributed Telemetry Collector (Data Flywheel)
═══════════════════════════════════════════════════════════════════
Every SDK instance is a sensor. This module collects anonymized
routing decisions and ships them to Conrrad Cloud for continuous
...
logger = logging.getLogger("conrrad.router.telemetry")
...
class TelemetryConfig:
    ...
    endpoint: str = "https://telemetry.conrrados.com/v1/events"
...
class TelemetryCollector:
    ...
    It IS the competitive advantage:
      - Every SDK instance feeds routing decisions to Conrrad Cloud
    ...
    def enable(self, consent: bool = True) -> None:
        ...
        if consent:
            ...
            logger.info(
                ...
                "Anonymized routing data will be sent to Conrrad Cloud "
    ...
    def _flush(self) -> None:
        """Ship buffered events to Conrrad Cloud."""
        with self._lock:
            if not self._buffer:
                return
    ...
    def _start_flush_thread(self) -> None:
        ...
        thread = threading.Thread(target=_loop, daemon=True, name="conrrad-telemetry")
        thread.start()
