...
from conrrad_sdk.escrow.manager import EscrowManager, EscrowState
from conrrad_sdk.runtime.firecracker.auth_protocol import AuthenticatedFrame
from conrrad_sdk.runtime.firecracker_runtime import FirecrackerRuntime
from conrrad_sdk.runtime.models import ExecutionRequest
from conrrad_sdk.runtime.firecracker import server as firecracker_server
...
def test_response_context_mismatch_rejected(monkeypatch):
    ...
    monkeypatch.setattr("conrrad_sdk.runtime.firecracker_runtime.socket.socket", _FakeSocket)
    monkeypatch.setattr("conrrad_sdk.runtime.firecracker_runtime.prom.VSOCK_CONNECT_LATENCY.observe", lambda *_: None)
    monkeypatch.setattr(
        "conrrad_sdk.runtime.firecracker_runtime.recv_len_prefixed",
        lambda _sock: b'{"v":1,"ts":1,"nid":"x","tid":"attacker","rid":"wrong","pay":"","meta":{},"sig":"x"}',
    )
    monkeypatch.setattr(
        "conrrad_sdk.runtime.firecracker_runtime.AuthenticatedFrame.from_wire",
...
def test_every_escrow_transition_emits_event():
    ...
    with patch("conrrad_sdk.escrow.manager._now", return_value=1.0):
...
def test_no_silent_pass_in_critical_security_modules():
    modules = [
        firecracker_server,
        __import__("conrrad_sdk.runtime.firecracker.auth_protocol", fromlist=["*"]),
        __import__("conrrad_sdk.runtime.firecracker_runtime", fromlist=["*"]),
        __import__("conrrad_sdk.escrow.manager", fromlist=["*"]),
    ]
    for mod in modules:
