...
from conrrad_sdk.router.types import (
    SubTask, ExecutionResult, DifficultyLevel, ModelTier, TaskDomain, RouterStats,
)
from conrrad_sdk.router.decomposer import TaskDecomposer, DecomposerTrainingCollector
from conrrad_sdk.router.verifier import SelfVerifier, VerificationResult
from conrrad_sdk.router.summarizer import RollingSummarizer
from conrrad_sdk.router.model_registry import ModelRegistry, DEFAULT_CATALOG
from conrrad_sdk.router.metrics import RouterMetricsCollector
from conrrad_sdk.router.estimator import CostEstimator
from conrrad_sdk.router.intelligent_router import IntelligentRouter
from conrrad_sdk.router.entrypoint import RouterEntrypoint, RouterConfig
...
class TestShadowMode:
    ...
    def test_shadow_survives_intelligent_crash(self):
        ...
        result = entry.route("task", messages=[{"role": "user", "content": "test"}])
        assert result == "safe_result"  # Historical must still work
