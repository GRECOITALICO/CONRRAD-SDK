...
python3 -c "
from conrrad_sdk.shadow import ShadowProxy, ShadowConfig
proxy = ShadowProxy(ShadowConfig(agent_id='$AGENT_ID'))
print('   ✅ Shadow Proxy OK')
print(f'   ✅ Log dir: {proxy._config.log_dir}')
" 2>/dev/null || {
    echo -e "${RED}❌ Verification failed. Try: pip install -e conrrad-sdk --force-reinstall${NC}"
    exit 1
}
...
echo -e "  ${GREEN}from conrrad_sdk.shadow.proxy import patch_openai${NC}"
